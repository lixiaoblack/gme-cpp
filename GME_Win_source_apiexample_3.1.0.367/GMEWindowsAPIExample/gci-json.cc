...
void Json::DoDeepCopy(const Json& rhs) {
  ...
  switch (kind_) {
    case kNull:
      ...
      break;
    case kNumber:
      data_ = new double(*static_cast<const double*>(rhs.data_));
      break;
    case kString:
      data_ = new string(*static_cast<const string*>(rhs.data_));
      break;
    case kBool:
      data_ = new bool(*static_cast<const bool*>(rhs.data_));
      break;
    case kArray: {
...
void Json::Release() {
  ...
  switch (kind_) {
    case kNumber:
      ...
      break;
    case kString:
      delete static_cast<string*>(data_);
      break;
    case kBool:
...
bool Json::IsEmpty() const {
  if (IsObject()) {
    return CAST_JSON_OBJ(data_)->size() == 0;
  }
  if (IsArray()) {
    return CAST_JSON_ARR(data_)->size() == 0;
  }
  return false;
}
...
int Json::Size() const {
  ...
  return (int)CAST_JSON_ARR(data_)->size();
}
...
string Json::ToString() const {
  ...
  switch (kind_) {
    case kNumber: {
      int nTemp = (int)(*static_cast<double*>(data_));
      oss << std::fixed << nTemp;
    } break;
    case kString:
      oss << "\"" << *static_cast<string*>(data_) << "\"";
      break;
    case kBool:
      oss << boolalpha << *static_cast<bool*>(data_);
      break;
    case kNull:
      oss << "null";
      break;
...
Json* Json::Parser::ConsumeValue(bool section /* = true */) {
  ...
  switch (kind) {
    case kNumber: {
      consumer = &Json::Parser::ConsumeNumber;
      break;
    }
    case kString: {
      consumer = &Json::Parser::ConsumeString;
      break;
    }
    case kBool: {
      consumer = &Json::Parser::ConsumeBool;
      break;
    }
    case kNull: {
      consumer = &Json::Parser::ConsumeNull;
      break;
    }
    case kObject: {
      consumer = &Json::Parser::ConsumeObject;
      break;
    }
    case kArray: {
      consumer = &Json::Parser::ConsumeArray;
      break;
    }
    default: {
      throw;
...
Json* Json::Parser::ConsumeBool() {
  ...
  Retract();
  if ('t' == ch) {
    ConsumeSpecific("true");
    return new Json(true);
  } else {
    ConsumeSpecific("false");
...
void Json::Parser::ConsumeSpecific(const char* str) {
  ...
  int len = (int)char_traits<char>::length(str);
  for (int i = 0; i < len; ++i) {
    if (str[i] != NextCharacter()) {
      UnexpectedToken();
    }
  }
...
Json::Kind Json::Parser::KindDetect() {
  ...
  switch (NextCharacter()) {
    ...
    default:
      break;  // treat as number (will cause exception)
  }
  Retract();
  return kind;
}
...
void Json::Parser::UnexpectedToken() {
  throw Json::UnexpectedTokenException(character, pos);
}

/* Json::UnexpectedTokenException */
Json::UnexpectedTokenException::UnexpectedTokenException(char ch, int pos) : exception(), ch_(ch), pos_(pos) {
  ostringstream oss;
  if ('\0' == ch_) {
